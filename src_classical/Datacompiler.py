...
def generate_compiler(data_root, hdf_data_path,BZ,IR,HARDSTOP,
    ...
    for filename in os.listdir(data_root):
        ...
        if len(files[0]) >= HARDSTOP * IR and len(files[1]) >= HARDSTOP:
            break
    ...
    gen = DataGenerator(data_root, files, hdf_data_path,  BZ, IR, HARDSTOP)
...
class DataGenerator:
    def __init__(self, data_root, file_list, hdf_data_path, BZ, IR, HARDSTOP, n_classes=2):
        # Basic properties:
        ...
        "test"
        self.HARDSTOP = HARDSTOP if HARDSTOP != float('inf') else self.target_len
