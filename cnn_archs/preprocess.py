...
def preprocess_classical(data_root, HARDSTOP, BZ, hdf_data_path = 'DL_info/chip_info/cube_raw', IR = 1):
    n_classes = 2
...
def preprocess_resized(data_root, HARDSTOP, BZ, hdf_data_path = 'DL_info/chip_info/cube_raw', IR = 1):
    n_classes = 2
    file_list = [[] for clas in range(n_classes)]
    ...
    for i, data in enumerate(dldr):
        ...
        if i < 2 * HARDSTOP:
            ...
            target_shape = (32, 32, 50)
            zoom_factors = (target_shape[0] / 64,
                target_shape[1] / 64,
                target_shape[2] / 101)
            
            inputs = zoom(inputs, zoom_factors, order=3) 
