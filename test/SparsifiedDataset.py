...
def balance(image_list, label_list, hardstop, IR, wires, bz):
    ...
    for i in range(len(image_list)):
        ...
        if label == 0 and _info[label] < hardstop * IR:
            _info[label] += 1
            image_list_temp[i] = np.resize(image_list[i], (wires, 101, 64, 64))
...
class QuantumDataset(Dataset):
    def __init__(self, image_list, label_list, hardstop, IR = 1):
        ...
        # print(hardstop)
        self.image_list, self.label_list = balance(image_list, label_list, hardstop, IR, 4, 20)
